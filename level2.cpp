...
void resetLevel2() {
    ...
    platforms.push_back({0, -250, 100, 0, 0});                             // first platform
    for(int i = 0; i < 10; i++)
        platforms.push_back({
...
void drawLevel2() {
    for (auto& plat : platforms) {                                         //Loop through every platform
        if (plat.bounces >= 3) continue;                                   // If jumped on 3 times it vanished
        ...
        glTranslatef(plat.x, plat.y, 0);                                   //move to platform's position (Translation)
        glScalef(currentWidth / 100.0f, 1.0, 1.0);                         //resize the platform width (Scaling)
